...
**Last Updated:** January 25, 2026 — Task 3 **complete**: `POST /api/agent/interact` implemented (§4.2); tasks/task_actions models, RAG helper, LLM integration  
...
**Tenant:** In normal mode, tenant = **user** (`userId`). In organization mode, tenant = **organization** (`organizationId`). No separate `tenants` table. See `getTenantState` / `getActiveOrganizationId` and `ENTERPRISE_PLATFORM_SPECIFICATION.md` §1 (Multi-Tenant Architecture & Security).
...
**Deliverable:** Login, session, and logout work; `getSessionFromToken` is used by all protected routes. Client integration is in `THIN_CLIENT_ROADMAP.md` §2.
...
- **`POST /api/v1/auth/google`** (Google OAuth)  
  ...
  - Initiates Google OAuth flow; returns authorization URL for extension to open in popup/tab.  
  - Response: `{ authUrl: string }`. Extension opens this URL; user completes OAuth; callback returns token.  
  - **Implementation:** `app/api/v1/auth/google/route.ts` — wraps Better Auth social sign-in, returns OAuth URL with CORS.

- **`GET /api/v1/auth/google/callback`** (Google OAuth callback)  
  - Query params: `code`, `state` (from Google OAuth redirect).  
  - Completes OAuth flow; exchanges code for token; creates/updates user.  
  - Response: `{ accessToken, expiresAt, user: { id, email, name }, tenantId, tenantName }`.  
  - **Implementation:** `app/api/v1/auth/google/callback/route.ts` — handles OAuth callback, extracts Bearer token, returns contract response with CORS.
...
**Google OAuth Configuration for Chrome Extension:**

**Important:** Chrome extensions don't have a traditional URL, but Google OAuth still works because:

1. **Authorized JavaScript Origins** (in Google Cloud Console):
   - Use your **web server's URL**, not the extension URL
   - Example: `https://yourdomain.com` or `http://localhost:3000` (dev)
   - **Why:** The OAuth flow is initiated from your server (`POST /api/v1/auth/google`), not directly from the extension. The extension calls your server, and your server initiates OAuth with Google.

2. **Authorized Redirect URIs** (in Google Cloud Console):
   - Use your **server's callback URL**, not the extension URL
   - Example: `https://yourdomain.com/api/v1/auth/google/callback` or `http://localhost:3000/api/v1/auth/google/callback` (dev)
   - **Why:** Google redirects back to your server after authentication, not to the extension. Your server then returns the token to the extension via JSON.

3. **Flow:**
   ```
   Extension → POST /api/v1/auth/google (your server)
   ↓
   Your Server → Google OAuth (initiates flow)
   ↓
   Google → Redirects to /api/v1/auth/google/callback (your server)
   ↓
   Your Server → Returns JSON with accessToken to Extension
   ```

4. **Better Auth `trustedOrigins` vs Google OAuth Origins:**
   - **Better Auth `trustedOrigins`**: Allows extension to make API calls (CORS). Uses `chrome-extension://<id>`.
   - **Google OAuth Origins**: Where OAuth flow happens. Uses your web server URL (`https://yourdomain.com`).

**Configuration Required:**
- `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` environment variables
- In Google Cloud Console, add:
  - **Authorized JavaScript origins**: `{BETTER_AUTH_URL}` (e.g., `https://yourdomain.com`)
  - **Authorized redirect URIs**: `{BETTER_AUTH_URL}/api/v1/auth/google/callback`
...
- [ ] End-to-end: extension can log in, session check, logout (see client roadmap). QA verifies on **live site**. **Pending:** Client-side implementation in `THIN_CLIENT_ROADMAP.md`.
...
**Objective:** Action loop endpoint. Backend receives `url`, `query`, `dom`, optional `taskId`; validates tenant and domain; loads or creates task; runs RAG; builds prompt with **server-held action history**; calls LLM; returns **`NextActionResponse`**. Client executes actions and loops (see `THIN_CLIENT_ROADMAP.md` §4).
...
- **`THIN_CLIENT_ROADMAP.md`** — Extension integration for Tasks 1–3.  
