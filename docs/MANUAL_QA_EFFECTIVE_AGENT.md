...
## Technical Prerequisites (Stability Fixes — Jan 31, 2026)

Before running Level 1+ tests, ensure the following stability fixes are in place:

### DOM Extraction After Navigation

**Issue:** Task 1.1 ("Go to Google, search for SpaceX...") was failing with `DOM extraction failed: Cannot read properties of null (reading 'querySelectorAll')` after the `navigate()` action.

**Fixes Applied:**
- `src/pages/Content/getAnnotatedDOM.ts`: Added null guards in `traverseDOM`, `traverseWithShadowDOM`, iframe handling
- `src/helpers/pageRPC.ts`: Increased tab load wait (500ms → 10s), more ping retries after injection

**Verification:** The "Go to Google" step should complete and DOM should be extractable within ~15 seconds of navigation.

### Real-Time Sync Stability

**Issue:** Excessive API calls causing 429 rate limits; "Cannot assign to read only property" errors on session updates.

**Fixes Applied:**
- `src/services/messageSyncService.ts`: Sync deduplication, debounced `handleInteractResponse`
- `src/services/pollingFallbackService.ts`: Array mutation fix (`[...array].sort()`)
- `src/state/store.ts`: Deep clone sessions in merge function

**Verification:** No 429 errors in console during normal operation; session switching doesn't cause errors.

---

## Changelog

- **Jan 31, 2026:** Added Technical Prerequisites section for stability fixes (DOM extraction, real-time sync).
